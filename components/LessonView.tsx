...
export const LessonView: React.FC<LessonViewProps> = ({ lesson, subjectType, subjectTitle, onComplete, onBack }) => {
  ...
  const playTTS = async (text: string) => {
    ...
    try {
        ...
        console.warn("Trình duyệt/Thiết bị không hỗ trợ đọc văn bản hoặc có lỗi xảy ra.", e);
    }
  }
  ...
  const loadQuiz = async () => {
      ...
      if (lesson.quizzes && lesson.quizzes.length > currentQuizIndex) {
          setQuizData(shuffleQuiz(lesson.quizzes[currentQuizIndex]));
          return;
      }

      // Fallback if no quizzes
      setQuizData({
                        ...
                        <div className="w-full max-w-md relative">
                            ...
                            <div className="bg-white border-4 border-indigo-50 shadow-xl rounded-3xl p-6 mb-6 relative min-h-[240px] flex flex-col items-center justify-center text-center">
                                ...
                                {quizData.illustration && (
                                    <div className={`mb-4 mt-4 w-full flex justify-center px-2 ${/[+\-=<>:×÷]/.test(quizData.illustration) ? 'overflow-x-auto' : ''}`}>
                                        {quizData.illustration.startsWith('http') ? (
                                            <img src={quizData.illustration} alt="minh họa" className="h-32 object-contain rounded-lg mx-auto max-w-full" />
                                        ) : (
                                            <span 
                                                className={`animate-bounce-small inline-block text-center ${
                                                    /[+\-=<>:×÷]/.test(quizData.illustration) ? 'whitespace-nowrap' : 'break-all'
                                                } ${
                                                    Array.from(quizData.illustration).length > 15 ? 'text-base sm:text-lg md:text-xl tracking-tighter' :
                                                    Array.from(quizData.illustration).length > 10 ? 'text-lg sm:text-xl md:text-2xl tracking-tight' :
                                                    Array.from(quizData.illustration).length > 6 ? 'text-2xl sm:text-3xl md:text-4xl' :
                                                    Array.from(quizData.illustration).length > 4 ? 'text-3xl sm:text-4xl md:text-5xl' :
                                                    'text-4xl sm:text-5xl md:text-6xl'
                                                }`}
                                                style={{ wordBreak: /[+\-=<>:×÷]/.test(quizData.illustration) ? 'normal' : 'break-all' }}
                                            >
                                                {quizData.illustration}
                                            </span>
                                        )}
                                ...
                                {quizData.options.map((opt, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className="flex gap-2">
                                            ...
                                            >
                                                <div className="w-10 h-10 shrink-0 rounded-full bg-indigo-50 border border-indigo-100 flex items-center justify-center text-lg font-black text-indigo-400">
                                                    {String.fromCharCode(65 + idx)}
                                                </div>
                                                <span className="text-2xl md:text-3xl font-medium break-words">{opt}</span>
                                            </button>
